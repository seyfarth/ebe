...
%token <QVariant_t> INT
%token <QVariant_t> FLOAT
%token <QVariant_t> BOOL
%token <QVariant_t> STRING
%token <QVariant_t> ID

// Define the tokens for the pseudo-ops
%token DB
%token DW
%token DD
%token DQ
...
%type <QVariant_t> start
%type <QVariant_t> file
%type <QVariant_t> command
%type <QVariant_t> instruction
%type <QVariant_t> value
%type <QVariant_t> expr
...
start: file {qDebug() << "done";};

file: file command      { qDebug() << "file command"; }
    | command
    ;
...
segment_selection:
      SEGMENT TEXT      { qDebug() << "text segment"; }
    | SEGMENT DATA      { qDebug() << "data segment"; }
    | SEGMENT BSS       { qDebug() << "bss segment"; }
    ;
...
values:
      values ',' expr
    | expr              { qDebug() << "expr" << $1->toInt(); }
    ;

expr:
      INT               { qDebug() << "INT value" << $1->toInt(); }
    | FLOAT
    | STRING
    | ID
    | '$'
