...
void CppHighlighter::highlightBlock(const QString &text)
{
    ...
    if (t[i] == QChar('/') && t[i + 1] == QChar('/')) goto CPPComment;
    if (t[i] == QChar('/') && t[i + 1] == QChar('*')) goto CComment;
    ...
    Illegal: setFormat(i, 1, illegalFormat);
    i++;
    goto More;

    Operator: setFormat(i, 1, operatorFormat);
    i++;
    goto More;

    Preprocessor: start = i;
    end = n - 1;
    ...
    : Highlighter(parent)
{
    keywords << "allocatable" << "allocate" << "assign" << "assignment"
        << "blockdata" << "block" << "data" << "call" << "case" << "character"
        << "common" << "complex" << "contains" << "continue" << "cycle"
        << "data" << "deallocate" << "default" << "do" << "doubleprecision"
        << "double" << "precision" << "else" << "elseif" << "elsewhere"
        << "endblockdata" << "blockdata" << "end" << "enddo" << "endfunction"
        << "endif" << "endinterface" << "endmodule" << "endprogram"
        << "endselect" << "endsubroutine" << "endtype" << "endwhere" << "entry"
        << "equivalence" << "exit" << "external" << "function" << "goto" << "go"
        << "to" << "if" << "implicit" << "in" << "inout" << "integer"
        << "intent" << "interface" << "intrinsic" << "kind" << "len"
        << "logical" << "module" << "namelist" << "nullify" << "only"
        << "operator" << "optional" << "out" << "parameter" << "pause"
        << "pointer" << "private" << "program" << "public" << "real"
        << "recursive" << "result" << "return" << "save" << "select" << "case"
        << "selectcase" << "stop" << "subroutine" << "target" << "then"
        << "type" << "use" << "where" << "while" << "backspace" << "close"
        << "endfile" << "file" << "format" << "inquire" << "open" << "print"
        << "read" << "rewind" << "write";
